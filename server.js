...
const app = express();
const PORT = Number.parseInt(process.env.TCP_PANEL_PORT || '8080', 10);
const HOST = process.env.TCP_PANEL_HOST || '0.0.0.0';
const BOT_TIMEOUT_SECONDS = Number.parseInt(process.env.BOT_TIMEOUT_SECONDS || '1800', 10);
...
function startBot() {
    ...
    try {
        ...
        botProcess = spawn(pythonPath, [scriptPath], {
            cwd: __dirname,
            env: { ...process.env, BOT_TIMEOUT_SECONDS: String(BOT_TIMEOUT_SECONDS) },
...
server.listen(PORT, HOST, () => {
    ...
    console.log(`Bot Control Panel: http://${HOST}:${PORT}/web.html`);
    console.log(`Current bot status: ${isRunning ? 'Running' : 'Stopped'}`);
    if (process.env.AUTO_START_BOT === '1') startBot();
});
